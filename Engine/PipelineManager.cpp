...
void PipelineManager::CreateAllPiplines(ComPtr<ID3D12Device> dev)
{
	// スプライトパイプライン生成
...
void PipelineManager::CreateGBufferPipline(ComPtr<ID3D12Device> dev)
{
	...
	(
		L"Assets/Shaders/3d/GBufferVS.hlsl",    // シェーダファイル名
		nullptr,
	...
	result = D3DCompileFromFile
	(
		L"Assets/Shaders/3d/GBufferPS.hlsl",    // シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
	...
	rootSignatureDesc.Init_1_0(_countof(rootparams), rootparams, 1, &sampler_desc, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
	...
	result = dev->CreateRootSignature(
		...
		rootSigBlob->GetBufferSize(), IID_PPV_ARGS(piplines_["GBuffer"].second.ReleaseAndGetAddressOf()));
	if (FAILED(result)) { assert(0); }

	// パイプラインにルートシグネチャをセット
	gpipeline.pRootSignature = piplines_["GBuffer"].second.Get();

	// グラフィックスパイプラインの生成
	result = dev->CreateGraphicsPipelineState(
		&gpipeline,
		IID_PPV_ARGS(piplines_["GBuffer"].first.ReleaseAndGetAddressOf()));
	if (FAILED(result)) { assert(0); }
...
void PipelineManager::CreateSkyBoxPipline(ComPtr<ID3D12Device> dev)
{
	...
	// 頂点シェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Assets/Shaders/SkyBoxVS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main", "vs_5_0",	// エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Assets/Shaders/SkyBoxPS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main", "ps_5_0",	// エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
	...
	// デプスステンシルステート
	gpipeline.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
	gpipeline.DepthStencilState.DepthFunc = D3D12_COMPARISON_FUNC_NEVER; // 常に上書きルール
	gpipeline.DepthStencilState.DepthEnable = false;					 // 上書きされる
	...
	result = dev->CreateRootSignature(
		0, rootSigBlob->GetBufferPointer(),
		rootSigBlob->GetBufferSize(), IID_PPV_ARGS(piplines_["SkyBox"].second.ReleaseAndGetAddressOf()));
	if (FAILED(result)) {
		assert(0);
	}
	// パイプラインにルートシグネチャをセット
	gpipeline.pRootSignature = piplines_["SkyBox"].second.Get();

	//グラフィックスパイプラインステートの生成
	result = dev->CreateGraphicsPipelineState(
		&gpipeline,
		IID_PPV_ARGS(piplines_["SkyBox"].first.ReleaseAndGetAddressOf()));
	if (FAILED(result)) {
		assert(0);
	}
...
void PipelineManager::CreatePostEffectPipline(ComPtr<ID3D12Device> dev, const std::string &pipline_name)
{
	...
	// 頂点シェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Assets/Shaders/PostEffectVS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main", "vs_5_0",	// エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
...
void PipelineManager::CreateMultiRenderTargetPipline(ComPtr<ID3D12Device> dev, const std::string &pipline_name)
{
	...
	// ピクセルシェーダの読み込みとコンパイル
	const std::string psFilename = "Assets/Shaders/PostEffectPS.hlsl";
	std::wstring psFilename_w = std::wstring(psFilename.begin(), psFilename.end());
...
void PipelineManager::CreateDeferredPpline(ComPtr<ID3D12Device> dev)
{
	...
	// 頂点シェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Assets/Shaders/3d/DeferredShaderVS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main", "vs_5_0",	// エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Assets/Shaders/3d/DeferredShaderPS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main", "ps_5_0",	// エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
	...
	result = dev->CreateRootSignature(
		0, rootSigBlob->GetBufferPointer(),
		rootSigBlob->GetBufferSize(), IID_PPV_ARGS(piplines_["Deferred"].second.ReleaseAndGetAddressOf()));
	if (FAILED(result)) {
		assert(0);
	}
	// パイプラインにルートシグネチャをセット
	gpipeline.pRootSignature = piplines_["Deferred"].second.Get();

	//グラフィックスパイプラインステートの生成
	result = dev->CreateGraphicsPipelineState(
		&gpipeline,
		IID_PPV_ARGS(piplines_["Deferred"].first.ReleaseAndGetAddressOf()));
	if (FAILED(result)) {
		assert(0);
	}
}

...
void FbxResource::FetchMaterial(FbxScene *fbx_scene, std::vector<Material> &materials)
{
	...
	for (size_t node_index = 0; node_index < node_count; ++node_index) {
		...
		for (int material_index = 0; material_index < material_count; ++material_index) {
			...
			// ディフューズのチェック
			FbxProperty fbx_property_diffuse = fbx_material->FindProperty(FbxSurfaceMaterial::sDiffuse);
			if (fbx_property_diffuse.IsValid())
			{
				const FbxDouble3 color{ fbx_property_diffuse.Get<FbxDouble3>() };
				...
				const FbxFileTexture *fbx_texture = fbx_property_diffuse.GetSrcObject<FbxFileTexture>();
			...
			{
				const FbxFileTexture *fbx_texture = fbx_property_normalmap.GetSrcObject<FbxFileTexture>();

...
std::vector<std::shared_ptr<FbxResource>>	DrawFbx::resouces_;
std::vector<std::vector<Fbx::Node>>			DrawFbx::nodes_;

DrawFbx *DrawFbx::GetIns()
{
	static DrawFbx *instans;
	return instans;
}

void DrawFbx::SetDrawResource(std::weak_ptr<FbxResource> resorces, std::vector<Fbx::Node> nodes)
{
	resouces_.emplace_back(resorces);
	nodes_.emplace_back(nodes);
}

void DrawFbx::AllDraw(Microsoft::WRL::ComPtr<ID3D12Device> dev, Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> cmd_list)
{

	int draw_num = 0;
	HRESULT result = S_OK;
	for(const auto &resouce : resouces_)
	{
		// メッシュ回数分処理する
